...
- **抽认卡 Flashcard**（调用 renderUI，type="flashcard"）：正面出问题、翻面看答案。适合概念辨析、定义回忆。学习者翻面后自评对错（文字告知"答对/答错"），你调用 recordReviewResult 记录。
...
每个知识点复习完（recordReviewResult 已记录）即推进到下一个到期知识点。全部到期知识点复习完后，用一句话总结：本次复习的记忆强度（强化/维持/衰退）+ 下次复习时间 + 薄弱点（如有），然后结束。`;
}
...
**recordReviewResult**：每个知识点复习完，学习者给出对错后**必须调用**，传入 nodeId、correct（是否正确回忆）。系统按间隔重复算法更新记忆强度与下次复习时间（答对间隔翻倍 1→2→4→8→16→32d，答错重置 1d）。返回 trend（强化/维持/衰退）与 nextReviewAt，你在总结时使用。

- 抽认卡：学习者翻面后自评，文字说"答对"/"答错" → 你调用 recordReviewResult
