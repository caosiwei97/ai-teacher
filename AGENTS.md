...
3. docs/开发/迭代计划.md    ← 迭代计划索引（版本总览 + 当前工作 + E2E 映射）
```
...
读取 `docs/开发/开发日志.md` 时，**首先检查顶部的 `## 待同步文档变更` 区域**：
...
如果日志末尾不明确，读取 `git log --oneline -10` 推断。
...
| 意图类型 | 识别特征 | 执行策略 | 文档同步 |
|---------|---------|---------|---------|
| **🟢 产品需求** | "加一个…"、"新增…"、"实现…功能" | 讨论方案 → 确认 → 实现 | API接口.md + 技术架构.md（视变更） |
| **🔴 问题修复** | "报错了"、"bug"、"不对" | 最小化修复 | 开发日志.md 记录 bug + 修复方案 |
| **🟡 技术改进** | "重构…"、"优化…"、"换方案" | 先讨论方案，确认后执行 | 决策记录.md（新增 ADR） |
| **🔵 探索调研** | "看看…"、"调研…"、"怎么做的" | 只读，不改代码 | 开发日志.md 记录结论 |
...
Agent 在执行任务过程中，如果发现文档内容与实际代码/项目结构不一致，**必须立即修正文档**，不能只在心里知道。发现不一致 → 立即修正 → 开发日志中记录 → 检查其他文档是否引用了相同过时信息。
