...
- 决策：不采用 NestJS（Agent 场景不适合），采用 Hono（轻量、AI SDK 兼容）
- 决策：不采用 LangGraph.js（TS 二等公民），自实现 StateGraph（借鉴 LangGraph 概念）
- 设计消息持久化 + BullMQ 队列 + Redis Pub/Sub 流式推送
- 设计 A2UI 框架（Tool-based UI + Component Registry）
- 选型 CodeMirror 6 替代 Monaco Editor，Judge0 作为代码沙箱
- 规划迭代 016-023（8 个迭代，详细任务拆分）
...
- **当前进度**：迭代 001-017 全部完成
- **下一步**：迭代 018（Agent 引擎 Phase 3：StateGraph）
- **注意**：24/24 E2E 测试全部通过，三进程架构运行正常（web:38421 + server:38422 + worker:38423）
- **架构升级文档**：[架构升级方案](../设计/架构升级方案.md)
