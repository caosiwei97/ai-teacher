...
> 状态：✅ 已完成 | 分类：🔵 功能 | 优先级：P1 | 依赖：018 | 完成日期：2026-05-09
...
- [x] 定义 `UIBlock` Zod discriminated union schema（shared 包）
- [x] 实现 `UIBlockRenderer` 前端组件（type → Component 映射）
- [x] 重构 AssessmentCard 为 UIBlock 渲染
- [x] 新增 InteractiveQuiz UIBlock 组件（可点击选项 + 即时反馈）
- [x] 新增 FormulaBlock UIBlock 组件（KaTeX LaTeX 渲染）
- [x] 安装 CodeMirror 6 依赖（@codemirror/view, @codemirror/state, @codemirror/lang-python, @codemirror/lang-javascript）
- [x] 实现 `CodeEditor` 组件（语言切换 + 语法高亮 + 紧凑模式）
- [x] 实现 `CodeResultBlock` UIBlock 组件（只读代码 + 输出面板）
- [x] CodeEditor 自定义暖色主题（匹配设计系统）
- [ ] Tool 输出自动映射到 UIBlock 格式
- [ ] Message metadata.uiBlocks 数组写入和读取
- [x] 文档更新：技术架构.md（A2UI 章节）、API接口.md（UIBlock schema）
