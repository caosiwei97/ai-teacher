...
> **实现口径校准（2026-07-01）**：当前生产实现位于
> `apps/worker/src/agent/context-manager.ts`。默认 6000 Token 是历史消息
> Compaction 的内部预算，不是模型上下文窗口；模型当前上下文占用以 Provider 返回的
> 最后一次 `totalTokens` 为准。GLM-4.7 的模型窗口按官方规格配置为 200K。
...
export class ContextManager {
  private tokenBudget: number; // 历史压缩预算，当前默认 6000；不是模型上下文窗口
    ...
    const compacted = [
      {
        type: "llm",
        role: "assistant",
        content: this.formatSummary(newSummary),
      },
      ...recent,
    ] as AgentMessage[];
    return {
...
- [x] 实现历史消息 Token 启发式估算；模型调用用量读取 Provider reported usage
- [x] 实现 Compaction 触发条件（estimatedHistoryTokens > compactionBudget \* 0.8）
