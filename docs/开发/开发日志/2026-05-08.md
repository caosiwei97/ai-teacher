...
**事件**：
- 新增快问功能：选中文本后出现"快问"按钮，基于选中内容提问，AI 流式回答
- 新增 AI 建议回复功能：输入框旁灯泡按钮，点击后获取思考方向提示
- 快问使用流式响应（streamText），建议回复使用一次性响应（generateText）

**代码变更**：
- `apps/web/src/app/api/quick-question/route.ts` — 新增快问 API（streamText）
- `apps/web/src/app/api/suggest-reply/route.ts` — 新增建议回复 API（generateText）
- `apps/web/src/components/chat/quick-question.tsx` — 新增快问浮动组件
- `apps/web/src/components/chat/chat-input.tsx` — 新增建议按钮和提示展示
- `apps/web/src/components/chat/chat-area.tsx` — 传递建议相关 props
- `apps/web/src/app/learn/[sessionId]/page.tsx` — 集成 QuickQuestion + 建议回复逻辑
- `e2e/quick-features.spec.ts` — 新增 API 级 E2E 测试
